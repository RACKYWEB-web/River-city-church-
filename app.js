...
/* ════════════════════════════════════════════
   AUTH SYSTEM — Sign In / Register / OTP
════════════════════════════════════════════ */

// Current user state (simulated)
let currentUser = JSON.parse(localStorage.getItem('nx-user') || 'null');
let pendingOtpEmail = '';
let generatedOTP = '';
let resendInterval = null;

// Show a specific auth step
window.showStep = (stepId) => {
  document.querySelectorAll('.auth-step').forEach(s => s.style.display = 'none');
  const el = document.getElementById(stepId);
  if (el) { el.style.display = 'block'; el.style.animation = 'stepIn .35s ease'; }
};
...
window.socialAuth = (provider) => {
  showToast(`Connecting to ${provider}…`);
  setTimeout(() => {
    simulateLogin({ name:'Alex Johnson', email:'alex@example.com', provider });
  }, 1200);
};
...
function simulateLogin(user) {
  currentUser = user;
  localStorage.setItem('nx-user', JSON.stringify(user));
  updateNavUser();
  document.getElementById('success-title').textContent = `Welcome, ${user.name.split(' ')[0]}! 🎉`;
  document.getElementById('success-msg').textContent = 'Your account is active. Explore the marketplace, courses, and AI tools.';
  showStep('auth-success');
...
function updateNavUser() {
  ...
  const icon = document.querySelector('.n-icon[onclick="openAuth()"]');
  if (icon) {
    icon.innerHTML = `<span style="font-size:11px;font-weight:700;color:var(--g2)">${currentUser.name.split(' ').map(n=>n[0]).join('').toUpperCase()}</span>`;
    icon.style.background = 'var(--gd)';
    icon.style.borderColor = 'var(--gb)';
...
window.doSignIn = () => {
  ...
  if (!pass || pass.length < 6)       { showToast('Please enter your password.','error'); return; }
  showToast('Signing you in…');
  setTimeout(() => simulateLogin({ name:'Alex Johnson', email }), 1000);
};
...
window.doRegister = () => {
  ...
  pendingUser = { name:`${fname} ${lname}`, email, role };
  sendOTP();
  showStep('auth-otp');
...
window.doForgot = () => {
  ...
  if (!email || !email.includes('@')) { showToast('Please enter your email address.','error'); return; }
  showToast('Password reset link sent! Check your inbox.');
  setTimeout(() => {
    document.getElementById('success-title').textContent = '📬 Check Your Email';
    document.getElementById('success-msg').textContent = `A reset link has been sent to ${email}. It expires in 15 minutes.`;
    showStep('auth-success');
  }, 800);
};
...
function sendOTP() {
  ...
  console.log(`%c🔐 OTP (demo): ${generatedOTP}`, 'color:#c8a84b;font-size:16px;font-weight:bold;');
  showToast(`📧 Verification code sent to ${pendingOtpEmail}`);
}
...
window.verifyOTP = () => {
  ...
  if (code.length < 6) { if(errEl){errEl.style.display='block';errEl.textContent='Please enter all 6 digits.';} return; }
  // Accept the real generated OTP or the demo code '123456'
  if (code === generatedOTP || code === '123456') {
    if(errEl) errEl.style.display='none';
    simulateLogin(pendingUser || { name:'New Member', email:pendingOtpEmail });
  } else {
    if(errEl){errEl.style.display='block';errEl.textContent='Incorrect code. Try again or use demo code: 123456';}
    [0,1,2,3,4,5].forEach(i=>{
      const b = document.getElementById('otp'+i);
      if(b){b.style.borderColor='rgba(239,68,68,.6)';setTimeout(()=>b.style.borderColor='var(--bd)',1500);}
    });
  }
};
...
function startResendTimer() {
  ...
  const count = document.getElementById('resend-count');
  if (!btn || !timer || !count) return;
  btn.style.display = 'none'; timer.style.display = 'inline';
  let sec = 60; count.textContent = sec;
  clearInterval(resendInterval);
  resendInterval = setInterval(() => {
    sec--; count.textContent = sec;
    if (sec <= 0) { clearInterval(resendInterval); btn.style.display='inline'; timer.style.display='none'; }
  }, 1000);
}
...
window.openCheckout = () => {
  // Require login first
  if (!currentUser) {
    showToast('Please sign in before checkout.','error');
    setTimeout(() => openAuth(), 400);
    return;
  }
  const bg = document.getElementById('checkout-bg');
  if (bg) bg.classList.add('open');
  goStep(1);
  ...
  if (ckEmail && currentUser.email) ckEmail.value = currentUser.email;
  if (ckFname && currentUser.name) {
    const parts = currentUser.name.split(' ');
    if (document.getElementById('ck-fname')) document.getElementById('ck-fname').value = parts[0]||'';
    if (document.getElementById('ck-lname')) document.getElementById('ck-lname').value = parts[1]||'';
  }
};
...
window.goStep = (n) => {
  [1,2,3,4].forEach(i => {
    const el = document.getElementById('ck-step'+i);
    if (el) el.style.display = (i===n) ? 'block' : 'none';
  });
  // Update step indicators
  document.querySelectorAll('.ck-step').forEach(s => {
    const sn = parseInt(s.dataset.step);
    s.style.opacity = sn <= n ? '1' : '0.4';
    const circle = s.querySelector('div');
    if (circle) {
      if (sn < n) { circle.style.background='linear-gradient(135deg,#10b981,#34d399)'; circle.textContent='✓'; }
      else if (sn === n) { circle.style.background='var(--gs)'; circle.textContent=sn; }
      else { circle.style.background='var(--bd)'; circle.textContent=sn; }
    }
  });
};
...
function populateCheckoutSummary() {
  ...
  const subEl   = document.getElementById('ck-subtotal');
  const totEl   = document.getElementById('ck-total-show');
  if (!itemsEl) return;
  const c = JSON.parse(localStorage.getItem('nx-cart') || '[]');
  if (c.length === 0) {
    itemsEl.innerHTML = '<div style="font-size:13px;color:var(--tm);text-align:center;padding:16px">No items in cart</div>';
    return;
  }
  itemsEl.innerHTML = c.map(item => `
    <div style="display:flex;align-items:center;gap:12px;padding:10px;background:var(--p);border:1px solid var(--bd);border-radius:var(--rs)">
      <div style="width:38px;height:38px;border-radius:8px;background:var(--b3);display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">${item.icon||'📦'}</div>
      <div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:600;color:var(--tw);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${item.name}</div><div style="font-family:var(--fm);font-size:10px;color:var(--tm);margin-top:2px">Digital Product</div></div>
      <div style="font-family:var(--fh);font-size:15px;font-weight:700;color:var(--g2);flex-shrink:0">${item.price}</div>
    </div>`).join('');
  const sub = c.reduce((s,i)=>s+parseFloat(String(i.price).replace(/[^0-9.]/g,'')),0);
  const tot = Math.max(0, sub - cartDiscount);
...
window.goPayment = () => {
  ...
  const country = document.getElementById('ck-country')?.value;
...
window.selectMethod = (el, method) => {
  document.querySelectorAll('.pay-method').forEach(m => m.classList.remove('on'));
  el.classList.add('on');
  document.getElementById('pm-card').style.display   = method==='card'   ? 'block':'none';
  document.getElementById('pm-paypal').style.display = method==='paypal' ? 'block':'none';
  document.getElementById('pm-crypto').style.display = method==='crypto' ? 'block':'none';
};

window.formatCard = (inp) => {
  let v = inp.value.replace(/\D/g,'').substring(0,16);
  inp.value = v.replace(/(.{4})/g,'$1 ').trim();
};

window.formatExpiry = (inp) => {
...
window.applyPromo = () => {
  ...
  const discRow = document.getElementById('ck-discount-row');
  const discEl  = document.getElementById('ck-discount');
  const totEl   = document.getElementById('ck-total-show');
  if (!code) { showToast('Please enter a promo code.','error'); return; }
  const codes = { 'NEXUS20':20, 'LEARN50':50, 'WELCOME10':10, 'VIP30':30 };
  if (codes[code]) {
    const pct = codes[code];
    const c = JSON.parse(localStorage.getItem('nx-cart')||'[]');
    const sub = c.reduce((s,i)=>s+parseFloat(String(i.price).replace(/[^0-9.]/g,'')),0);
    cartDiscount = (sub * pct/100);
    if(discRow) discRow.style.display='flex';
    if(discEl)  discEl.textContent='-$'+cartDiscount.toFixed(2);
    if(totEl)   totEl.textContent='$'+Math.max(0,sub-cartDiscount).toFixed(2);
    if(msg){ msg.style.display='block'; msg.style.color='var(--e3)'; msg.textContent=`✅ ${pct}% discount applied — you save $${cartDiscount.toFixed(2)}`; }
    showToast(`🎉 Promo applied! ${pct}% off`);
  } else {
    if(msg){ msg.style.display='block'; msg.style.color='#f87171'; msg.textContent='❌ Invalid code. Try: NEXUS20, LEARN50, WELCOME10'; }
    showToast('Invalid promo code.','error');
  }
};

window.goVerify = () => {
  const method = document.querySelector('.pay-method.on')?.dataset.method;
  if (method === 'card') {
    const num = document.getElementById('card-num')?.value?.replace(/\s/g,'');
    const exp = document.getElementById('card-exp')?.value;
    const cvv = document.getElementById('card-cvv')?.value;
    const name = document.getElementById('card-name')?.value?.trim();
    if (!num || num.length < 13) { showToast('Please enter a valid card number.','error'); return; }
    if (!exp || exp.length < 7)  { showToast('Please enter the expiry date.','error'); return; }
  ...
  currentPayOTP = Math.floor(100000 + Math.random() * 900000).toString();
  console.log(`%c💳 Payment OTP (demo): ${currentPayOTP}`, 'color:#00d4ff;font-size:16px;font-weight:bold;');
  const email = document.getElementById('ck-email')?.value || (currentUser?.email||'');
  const showEl = document.getElementById('ck-email-show');
  if(showEl) showEl.textContent = email;
  showToast(`📧 Payment code sent to ${email}`);
  goStep(3);
  [0,1,2,3,4,5].forEach(i=>{ const b=document.getElementById('cotp'+i); if(b) b.value=''; });
  setTimeout(()=>document.getElementById('cotp0')?.focus(),200);
};

// Payment OTP navigation
window.cotpNext = (el, idx) => {
  el.value = el.value.replace(/\D/g,'');
  if (el.value && idx < 5) document.getElementById('cotp'+(idx+1))?.focus();
  el.addEventListener('keydown', function(e){
    if (e.key==='Backspace' && !this.value && idx>0) document.getElementById('cotp'+(idx-1))?.focus();
  }, {once:true});
};

window.confirmPayment = () => {
  const code = [0,1,2,3,4,5].map(i=>document.getElementById('cotp'+i)?.value||'').join('');
  const errEl = document.getElementById('cotp-error');
  if (code.length < 6) { if(errEl){errEl.style.display='block';errEl.textContent='Please enter all 6 digits.';} return; }
  if (code === currentPayOTP || code === '123456') {
    if(errEl) errEl.style.display='none';
    // Clear cart
    localStorage.removeItem('nx-cart');
    cart = [];
    updateCartUI();
    // Show success
    const ref = 'NX-'+Date.now().toString(36).toUpperCase();
    const refEl = document.getElementById('ck-ref');
    if(refEl) refEl.textContent = 'Order Reference: '+ref;
    goStep(4);
    showToast('🎉 Payment confirmed! Check your email.');
  } else {
    if(errEl){errEl.style.display='block';errEl.textContent='Incorrect code. Try again or use demo: 123456';}
    [0,1,2,3,4,5].forEach(i=>{
      const b=document.getElementById('cotp'+i);
      if(b){b.style.borderColor='rgba(239,68,68,.6)';setTimeout(()=>b.style.borderColor='var(--bd)',1500);}
    });
  }
};
...
window.resendPayOTP = () => {
  currentPayOTP = Math.floor(100000+Math.random()*900000).toString();
  console.log(`%c💳 New Payment OTP: ${currentPayOTP}`,'color:#00d4ff;font-size:16px;font-weight:bold;');
  showToast('📧 New verification code sent!');
  [0,1,2,3,4,5].forEach(i=>{ const b=document.getElementById('cotp'+i); if(b) b.value=''; });
  document.getElementById('cotp0')?.focus();
};

// Override the old checkout function to use new modal
window.checkout = () => openCheckout();

// Show user profile menu after login
window.showUserMenu = () => {
  showToast(`Signed in as ${currentUser?.name || 'Member'}`);
};

// Check saved login on load
if (currentUser) {
  document.addEventListener('DOMContentLoaded', () => { setTimeout(updateNavUser, 500); });
}

/* ════════════════════════════════════════════
   BUY NOW — single item instant checkout
════════════════════════════════════════════ */
window.buyNow = (name, price, icon) => {
  // Add item to cart (replacing existing if same)
  const existing = JSON.parse(localStorage.getItem('nx-cart') || '[]');
  // Check if already in cart
  if (!existing.find(i => i.name === name)) {
    existing.push({ name, price, icon });
    localStorage.setItem('nx-cart', JSON.stringify(existing));
    cart = existing;
    updateCartUI();
  }
...
(function restoreCart() {
  const saved = JSON.parse(localStorage.getItem('nx-cart') || '[]');
  if (saved.length > 0) {
    cart = saved;
    // Update badge when DOM is ready
    document.addEventListener('DOMContentLoaded', () => {
      updateCartUI();
    });
  }
